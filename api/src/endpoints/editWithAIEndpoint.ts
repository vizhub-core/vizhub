...
export const editWithAIEndpoint = ({
  ...
  app.post(
    ...
    bodyParser.json(),
    async (req, res) => {
      debug && console.log('[editWithAIEndpoint] req.body:', req.body);
      ...
      try {
        ...
        if (!id || !prompt) {
          res.json(missingParameterError("Missing 'id' or 'prompt'"));
          return;
        }
        ...
        await new Promise<void>((resolve, reject) => {
          shareDBDoc.subscribe((error) => {
            if (error) {
              console.error('shareDBDoc.subscribe error:', error);
              reject(error);
              return;
            }
            resolve();
